...
for j = 1:length(Islands)
    ...
    if isempty(mpc_new.gen)
        P_Shed(j) = sum(mpc_new.bus(:,PD)); % amount of real power outage
        P_Served(j) = 0; % amount of real power remians in the system   
        Is_Converged(j) = 1;
    ...
    elseif ng == 1
        ...
        if D_i < S_i
            P_Shed(j) = 0;
            P_Served(j) = sum(mpc_new.bus(:,PD));
        else
            Diff_P = sum(mpc_new.bus(:,PD)) - mpc_new.gen(:,PMAX);
            Diff_P(Diff_P < 0) = 0;
            P_Shed(j) = sum(Diff_P);
            P_Served(j) = sum(mpc_new.gen(:,PMAX));
        end
        ...
        if results_pf.success == 1 % This 'if and else' tries to see whether opf converges or not.
            mpc_temp = mpc_array{j};
            
            P_Shed(j) = sum(mpc_temp.bus(:,PD)) - sum(mpc_new.bus(:,PD));
            P_Served(j) = sum(mpc_new.bus(:,PD));
            
            Is_Converged(j) = 1;
        else
            mpopt_new = mpoption(mpopt, 'opf.ac.solver', 'MIPS', 'mips.step_control', 1,'mips.sc.red_it', 100, 'mips.max_it', 500);
            results_pf = runpf(mpc_new,mpopt_new);
        ...
        if  results_pf.success == 0   % After the previous steps, if still opf does not converge, algorithm goes into this 'if statement'
            if ~isempty(overloaded)
                ...
                if results_pf.success == 1 || isempty(overloaded) 
                    mpc_temp = mpc_array{j};
...
II = 0;
for j = (length(Islands)+1):(length(Islands)+length(Isolated))
    II = II + 1;
    MPC{j} = Isolated(II);
    ...
    if ismember(mpc.bus(Isolated(II),BUS_I),mpc.gen(:,GEN_BUS))
        ...
        if D_isolated < S_isolated
            P_Shed(j) = 0;
            P_Served(j) = mpc.bus(ind_bus_isolated,PD);
        else
            P_Shed(j) = mpc.bus(ind_bus_isolated,PD) - mpc.gen(ind_gen_isolated,PMAX);
            P_Served(j) = mpc.gen(ind_gen_isolated,PMAX);
        end
        
    else
        P_Shed(j) = mpc.bus(ind_bus_isolated,PD);
        P_Served(j) = 0; % amount of real power remians in the system
    end
