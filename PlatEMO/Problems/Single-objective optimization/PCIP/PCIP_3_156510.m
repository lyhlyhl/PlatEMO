classdef PCIP_3_156510 < PROBLEM
% <single> <integer>
    properties
        conflictMarix_11 
        disturbMarix_11
    ...
    methods
        ...
        function Setting(obj)
            obj.M = 1;
            if isempty(obj.D); obj.D = 454; end
            ...
            %数据读取
            obj.conflictMarix_11 = load("fre156510.mat").fre156510_con_11;%冲突矩阵
            obj.disturbMarix_11 = load("fre156510.mat").fre156510_dis_11;%干扰矩阵
            obj.conflictMarix_01 = load("fre156510.mat").fre156510_con_01;%冲突矩阵
            obj.disturbMarix_01 = load("fre156510.mat").fre156510_dis_01;%干扰矩阵
            %这里还需要修改
            obj.PCI454_0 = load("PCI.mat").PCI156510_0; %获得156510频率的PCI

            obj.fre454_pos = load("PCI.mat").fre454_pos; %获得454的PCI序号，方便修改
            
            %链表实现以缩减稀疏矩阵的运算量
            obj.Node = struct('data', [], 'next', []);
            obj.confligLinklistMarix = repmat(obj.Node, 1, 454);
            disp(max(obj.conflictMarix_01))
            for i = 1:454 %头结点矩阵
                disp(i)
                newhead = obj.createLinkedList(i); %建立新节点
                for j = 1:14370 %寻找所有连接节点——冲突不为0即连接
                    if obj.conflictMarix_01(i,j) ~= 0 
                        disp(55555)
                        obj.insertNode(newhead, obj.PCI454_0(j));
                    end
                end 
